...
async def get_detail_names(json: dict, detail: str, key: str):  # пример 'films': ['https://swapi.dev/api/films/1/']
    """переходим на films, species, starships, vehicles и уточняем список их названий для героя"""
...
async def get_homeworld(json: dict):  # пример 'homeworld': 'https://swapi.dev/api/planets/1/'
    """переходим на planet и уточняем  название homeworld для героя"""
...
async def main():
    ...
    people = await asyncio.gather(*people_tasks)  # результат - список с json по каждому герою
